"""
detect_whitespaces.py
─────────────────────
Step 5b — DBSCAN White Space / Untapped Pocket Detector.

Finds dense clusters of Independent Workshops that have no Dealer
within 15 km — these are "white spaces" with proven vehicle maintenance
demand but zero authorized dealer coverage.

Run from the ml-engine/ folder:
    python detect_whitespaces.py

Outputs:
    ../data/processed/whitespaces.csv   — one row per white-space pocket
    models/dbscan_model.pkl             — saved DBSCAN model
    Updates is_white_space = TRUE on relevant market_potential_grid rows

LESSONS LEARNED:
  - Previous run found 0 white spaces because:
      1. Workshops and dealers were generated from same cities → max gap 8.9 km
      2. Rural jitter ±0.3° spread workshops over 66×66 km → too sparse to cluster
  - Both fixed in generate_locations.py:
      → 39 sub-clusters with ±0.06° jitter guarantees 39 white spaces
  - ST_Y(geom) = latitude, ST_X(geom) = longitude
  - Strip +asyncpg from DATABASE_URL for psycopg2
  - DBSCAN eps must be in RADIANS when using haversine metric
"""
...
if ws_df.empty:
    ...
    for _, row in ws_df.iterrows():
        ...
        """, (
            float(row["centroid_lon"]),   # cast to Python float — psycopg2 rejects np.float64
