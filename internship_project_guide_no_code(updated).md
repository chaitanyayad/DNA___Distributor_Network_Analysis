...
- **Python 3.11+** — for data generation, cleaning, the ML engine, and the backend API server
...
- **fastapi** — the web framework for building the REST API backend
- **uvicorn** — the ASGI server that runs FastAPI in development and production
- **python-jose** — creates and verifies JWT authentication tokens
- **passlib & bcrypt** — hashes user passwords before storing in the database
- **python-dotenv** — loads secret keys from a `.env` file instead of hardcoding them
- **asyncpg** — high-performance async driver that connects FastAPI to PostgreSQL
...
**What you are doing:** Building a REST API server using Python FastAPI that sits between the database and the frontend. The frontend never talks directly to the database — it always goes through the API.

**Why this matters:** The API is where all business logic lives — authentication, permission checks, spatial conflict validation. It protects the database from direct access and ensures business rules are enforced consistently. FastAPI is the right choice here because the entire project is already Python-based (data generation, ML engine, database loading) — there is no context switch, you can share utility functions across the backend and ML code, and FastAPI's automatic interactive documentation (Swagger UI at `/docs`) makes testing endpoints during development dramatically faster.

### Why FastAPI Specifically

FastAPI gives you three things that matter for this project:

**Automatic API documentation** — The moment you define an endpoint, FastAPI generates a live, interactive Swagger UI at `http://localhost:8000/docs`. You can test every endpoint directly in the browser without Postman during early development. This is genuinely useful when building spatial endpoints that have complex query parameters.

**Built-in data validation via Pydantic** — Every request body and response is defined as a Pydantic model (a Python class with typed fields). If the React frontend sends a malformed polygon or a missing field, FastAPI automatically returns a clear 422 validation error before your business logic even runs. You don't have to write manual input checks.

**Async support** — FastAPI is built on async Python. Combined with the `asyncpg` database driver, database queries run asynchronously, meaning the server can handle multiple map requests from multiple users simultaneously without blocking.

### FastAPI Project Structure

Inside the `backend/` folder, organize the FastAPI application as follows:

```
backend/
├── main.py                  # App entry point — registers all routers, sets up CORS
...
│   ├── user.py
│   └── workflow.py
├── routers/                 # One file per feature area (equivalent to Express routes)
│   ├── auth.py
│   ├── locations.py
│   ├── territories.py
│   ├── analytics.py
...
Start the development server using Uvicorn (the ASGI server that runs FastAPI):

```
uvicorn backend.main:app --reload --port 8000
```

The `--reload` flag means the server automatically restarts whenever you save a file — essential during development. The API will be live at `http://localhost:8000` and the interactive docs at `http://localhost:8000/docs`.

### API Endpoints to Build

**Authentication (`/api/auth`):**
- `POST /api/auth/login` — accepts email + password in the request body, verifies the password against the stored bcrypt hash, returns a JWT token if credentials are valid. Define a Pydantic model for the login request body so FastAPI validates the input automatically.
- `POST /api/auth/logout` — invalidates the session on the frontend (since JWTs are stateless, the backend simply instructs the frontend to discard the token)

**Locations (`/api/locations`):**
- `GET /api/locations` — returns location records; accepts optional query parameters: `type` (entity type filter), `city`, and `bounds` (map viewport as `minLon,minLat,maxLon,maxLat`) so the map only fetches what is visible in the current viewport
- `GET /api/locations/{location_id}` — returns a single location's full details

**Analytics (`/api/analytics`):**
- `GET /api/analytics/hotspots` — returns market potential grid cells, filterable by a minimum `hotspot_score` query parameter
- `GET /api/analytics/whitespaces` — returns all identified white-space pockets with their opportunity scores
- `GET /api/analytics/summary` — returns aggregate counts and total revenue potential by region, used to populate the KPI cards on the admin dashboard

**Territories (`/api/territories`):**
- `GET /api/territories` — returns all territories for admins, or only the requesting distributor's territory for distributor users; the filtering happens inside the endpoint based on the role decoded from the JWT
- `POST /api/territories` — accepts a GeoJSON polygon in the request body; runs a PostGIS `ST_Intersects` spatial overlap check against all locked territories before saving; returns a 409 Conflict error with the conflicting territory's name if overlap is detected
- `PATCH /api/territories/{territory_id}/lock` — locks a territory; once locked it cannot be modified or overlapped

**Workflow (`/api/workflow`):**
- `POST /api/workflow/request-ro` — distributor submits a new Retail Office request with proposed coordinates; the backend runs a PostGIS `ST_Within` check to confirm the proposed point is inside the distributor's own territory before saving; rejects with a 400 error if the point is outside
- `GET /api/workflow/requests` — admin-only endpoint that returns all PENDING RO requests
- `PATCH /api/workflow/requests/{request_id}` — admin approves or rejects; if approved, automatically inserts a new record into the `locations` table

### Dependencies to Build (FastAPI's Equivalent of Middleware)

In FastAPI, reusable logic like authentication and RBAC is implemented as **dependency functions** rather than middleware. They work the same way conceptually but are injected directly into the function signature of each route that needs them.

**JWT verification dependency (`get_current_user`):** A dependency function that reads the `Authorization: Bearer <token>` header, verifies the JWT signature using the secret key, decodes the payload to extract the user's ID, role, and distributor ID, and returns a user object. Any route that declares this as a dependency automatically requires a valid token — FastAPI returns a 401 if the token is missing or invalid.

**RBAC dependency (`require_role`):** A higher-order dependency that takes a required role as a parameter and returns a dependency function that checks the current user's role against it. For example, the `POST /api/territories` route declares `require_role("org_admin")` as a dependency — FastAPI automatically returns 403 if a distributor user calls it, before the route's own logic executes.

**Database connection dependency (`get_db`):** A dependency that provides a database connection from the asyncpg connection pool to each route that needs it. The connection is automatically returned to the pool after the request completes, even if an exception occurs.

### Environment Variables

Create a `backend/.env` file (never commit this to Git — add it to `.gitignore`):

```
DATABASE_URL=postgresql://postgres:yourpassword@localhost:5432/distributor_gis
...
FastAPI reads these using `python-dotenv` via a `config.py` file that exposes them as typed settings using Pydantic's `BaseSettings`. This means if a required variable is missing from `.env`, the application refuses to start and tells you exactly which variable is missing — far better than a cryptic runtime error.

### One Advantage Specific to This Project

Because the ML engine (Step 5) is also Python, you can import your trained model files and utility functions directly into the FastAPI backend. For example, if a new grid cell's coordinates are submitted through the UI, the backend can run the XGBoost model in-process and return a live prediction without a separate ML service. This is not possible when the backend is Node.js.
...
**At the API level (the right way):** Every protected FastAPI endpoint declares `get_current_user` as a dependency. The `require_role` dependency is additionally declared on admin-only endpoints. The locations endpoint, for example, reads the distributor ID from the decoded JWT and automatically appends a `WHERE distributor_id = <user's distributor ID>` filter for distributor users, even if they try to pass different query parameters. This happens inside the route logic, not the frontend.

**At the UI level (user experience):** The React Router is configured with Protected Route components that check the user's role and redirect unauthorized users to a "403 Forbidden" page if they try to navigate to a route they shouldn't access. This is a UX convenience — the real enforcement is always at the API level.
...
| Backend API | EC2 t2.micro | 1 free instance, run FastAPI via Uvicorn here |
| PostgreSQL + PostGIS | RDS db.t3.micro | Managed database, automatic backups |
| File Storage (CSVs, model files) | S3 bucket | Free up to 5 GB; also store saved `.pkl` ML models here |
...
Create a `docker-compose.yml` file in the project root that defines three services: the database (using the official postgis/postgis Docker image), the FastAPI backend (run via Uvicorn inside the container), and the React frontend. Running `docker-compose up` starts the entire stack locally in one command — extremely useful for demos. Because the backend and ML engine are both Python, they can share the same Docker image if needed, keeping the setup simple.
...
4. Launch an EC2 instance, install Python 3.11, copy the backend code, install dependencies via `pip install -r requirements.txt`, set environment variables
5. Start the FastAPI server using Uvicorn with a process manager like `supervisor` or `systemd` so it restarts automatically if the instance reboots
6. Launch an RDS PostgreSQL instance, enable PostGIS, run your schema SQL
7. Load all data from your CSVs into the RDS database
8. Update the backend `.env` to point to the RDS endpoint instead of `localhost`
9. Update the React app's API base URL to point to the EC2 server's public IP
...
**API Reference:** FastAPI automatically generates interactive Swagger documentation at `/docs` — take a screenshot of this for your report as it visually proves every endpoint exists and is documented. Additionally, write a manual reference page listing each endpoint's purpose, required role, request parameters, and a sample response for clarity in the written report.
